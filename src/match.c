...
int matche (register char *p, register char *t, BOOLEAN case_sensitive)
{
      ...
      for ( ; *p; p++, t++)
      {
            ...
            case '*':                     /* multiple any character match */
                  return matche_after_star (p, t, case_sensitive);
...
int matche_after_star (register char *p, register char *t, BOOLEAN case_sensitive)
{
      ...
      do
      {
            ...
            /* if (nextp == *t || nextp == '[') */
            if ((case_sensitive ? nextp == *t : toupper(nextp) == toupper(*t)) || nextp == '[')
                  match = matche(p, t, case_sensitive);
...
BOOLEAN match( char *p, char *t, BOOLEAN case_sensitive )
{
      ...
      error_type = matche(p,t,case_sensitive);
